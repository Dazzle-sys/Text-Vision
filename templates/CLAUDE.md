...
- `list_windows()` — 列出当前打开的窗口(标题 + 进程名;含最小化窗口,标注"已最小化")
- `screen_capture(focus?, target?)` — 截取屏幕并描述。看当前屏幕直接 `screen_capture()`(若配置了 `VISION_DEFAULT_TARGET` 则默认截该窗口;需截全屏传 `target=''` 或 `'全屏'`/`'fullscreen'`);要截**指定程序窗口**(避免其他窗口遮挡),先 `list_windows()` 拿窗口清单,再 `screen_capture(target='进程名或标题')`
